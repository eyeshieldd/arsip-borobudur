    ...
    <section class="new-login-register" id="wrapper">
        ...
        <div class="new-login-box">
            ...
            <div class="white-box">
                ...
                <form class="form-horizontal new-lg-form" action="<?php echo base_url('auth/do_login') ?>" id="form-reset-password" onsubmit="return false" method="post">

                    <input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>"/>

                    <input type="hidden" name="user_id" value="<?= !empty($user_id)?$user_id:''; ?>" readOnly/>
                    ...
                    <div class="col-xs-12">
                        ...
                        <input class="form-control" placeholder="re enter password" name="re_password" required="" type="password">
                    </input>
                </div>
            </div>
            <div class="form-group text-center m-t-20">
                <div class="col-xs-12">
                    <button class="btn btn-info btn-lg btn-block btn-rounded text-uppercase waves-effect waves-light" type="submit" id="tombol-reset-password">
                        RESET
                    </button>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    <script>
        ...
        $('#form-reset-password').submit(function() { 
            ...
            var btn_submit_text = $(form + ' #tombol-reset-password').text(); 
            $(form + ' #tombol-reset-password').text('Submitted ...').attr('disabled', true);

            $.ajax({
                url: '<?=base_url('lupa_password/reset_password')?>',
                method: 'POST',
                data: $(form).serialize(), 
                timeout: 10000,
                dataType: 'json',
                success: function (data) {
                    if (data.status) {
                        ...
                        $(form + ' #tombol-reset-password').text(btn_submit_text).attr('disabled', false);
                    }
                },
