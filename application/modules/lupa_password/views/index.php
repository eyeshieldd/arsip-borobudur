...
<body>
    ...
    <section class="new-login-register" id="wrapper">
        ...
        <div class="new-login-box">
            <div class="white-box">
                ...
                <form class="form-horizontal new-lg-form" id="form-email-sent" onsubmit="return false" method="post">
            ...
            <div class="form-group text-center m-t-20">
                <div class="col-xs-12">
                                <!-- <a class="btn btn-info btn-lg btn-block btn-rounded text-uppercase waves-effect waves-light" href="<?=base_url('lupa_password/email_sent')?>">
                                    RESET
                                </a> -->
                                <button class="btn btn-info btn-lg btn-block btn-rounded text-uppercase waves-effect waves-light" id="btn-email-sent">RESET</button>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        $('#form-email-sent').submit(function() { 
            var form = '#form-email-sent';
            var btn_submit_text = $(form + ' #btn-email-sent').text(); 
            $(form + ' #btn-email-sent').text('Submitted ...').attr('disabled', true);

            $.ajax({
                url: '<?=base_url('lupa_password/send_email')?>',
                method: 'POST',
                data: $(form).serialize(), 
                timeout: 10000,
                dataType: 'json',
                success: function (data) {
                    if (data.status) {
                        alert(data.pesan, 'Success');
                        window.location.href = '<?=base_url('lupa_password/email_sent')?>';
                    } else {
                        alert(data.pesan, 'Error');
                        $(form + ' #btn-email-sent').text(btn_submit_text).attr('disabled', false);
                    }
                },
                error: function (textStatus) {
                    alert('An error occurred while connecting to the server. Error: ' + textStatus, 'Error');
                    $(form + ' #btn-email-sent').text(btn_submit_text).attr('disabled', false);
                }
            }); 
        }); 
